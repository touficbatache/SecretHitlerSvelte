      ...
      <div class="md:flex-1 w-full h-full flex flex-col">
          ...
          >
            <div class="self-stretch flex justify-between">
              <span class="flex items-center gap-2">
                <Icon class="md:text-xl" icon="fa6-solid:gamepad" /> Join & play ({pageDataJoinableGames.length})
              </span>
              <!-- TODO: invalidateAll() should work but it's not ==> maybe reactive data doesn't work with streamable content? -->
              <PlayfulIconButton
                extraClasses="w-8 h-7"
                icon="fa:refresh"
                on:click={() => location.reload()}
              />
            </div>
            {#if pageDataJoinableGames.length === 0}
            ...
            {:else}
              ...
              {#if pageDataJoinableGames.length > 2}
                <PlayfulButton
                  size="extra-small"
                  on:click={() => {
        ...
        {#if !expandJoinableGames}
          ...
          >
            <div class="self-stretch flex justify-between">
              <span class="flex items-center gap-2">
                <Icon class="md:text-xl" icon="fa6-solid:eye" /> Watch ({pageDataWatchableGames.length})
              </span>
              <!-- TODO: invalidateAll() should work but it's not ==> maybe reactive data doesn't work with streamable content? -->
              <PlayfulIconButton
                extraClasses="w-8 h-7"
            ...
            {#if pageDataWatchableGames.length === 0}
              <div class="my-auto flex flex-col items-center gap-3">
                <span>No&nbsp;one's&nbsp;playing&nbsp;right&nbsp;now, but&nbsp;you&nbsp;can:</span>
                <PlayfulButton size="extra-small" on:click={() => goto("/createGame")}
                  >Create a game</PlayfulButton
            ...
            {:else}
              ...
              {#if pageDataWatchableGames.length > 2}
                <PlayfulButton
                  size="extra-small"
                  on:click={() => {
